...
if (isset($_POST['title']) && isset($_POST['add_learning'])) {
 ...
 if ($_FILES['pdf_file']['error'] == 0 && $_FILES['pdf_file']['size'] > 0) {
    ...
    echo $sql = "INSERT INTO `tbl_learning` ( `category`, `language_id`, `title`, `video_id`, `detail`,pdf_file,`status`) VALUES ('" . $category . "','" . $language_id . "','" . $title . "','" . $video_id . "','" . $detail . "','" . $filename . "','0')";
    $db->sql($sql);
    $res = $db->getResult();
